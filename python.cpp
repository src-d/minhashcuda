...
static PyObject *py_minhash_cuda_init(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (iminit == init_methods.end()) {
      PyErr_SetString(
          ...
          "Unknown centroids initialization method. Supported values are "
              "\"minhash++\", \"random\" and <numpy array>.");
      return NULL;
    }
    init = iminit->second;
  } else {
  ...
  switch (result) {
    case kmcudaInvalidArguments:
      PyErr_SetString(PyExc_ValueError,
                      "Invalid arguments were passed to minhash_cuda");
      return NULL;
    ...
    case kmcudaRuntimeError:
      PyErr_SetString(PyExc_AssertionError, "minhash_cuda failure (bug?)");
      return NULL;
    case kmcudaSuccess:
      if (device_ptrs < 0) {
    ...
    default:
      PyErr_SetString(PyExc_AssertionError,
                      "Unknown error code returned from minhash_cuda");
      return NULL;
  }
#endif
}
